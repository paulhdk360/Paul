...
// Same upload/delete pattern again, for documents sent to a prospect
// (plaquette, devis, etc.) via the same polymorphic attachments table
// (link_type = "prospects", no affaire_id).
export async function uploadProspectAttachment(prospectId: string, formData: FormData) {
  const file = formData.get("file") as File | null;
  if (!file) throw new Error("Aucun fichier reçu.");
  if (file.size > MAX_FILE_SIZE) {
    throw new Error(`"${file.name}" dépasse ${Math.round(MAX_FILE_SIZE / (1024 * 1024))} Mo.`);
  ...
  const supabase = createClient();
  const storagePath = `prospects/${prospectId}/${crypto.randomUUID()}-${file.name}`;
  const { error: uploadError } = await supabase.storage
    .from("attachments")
    .upload(storagePath, file, { contentType: file.type || undefined });
  if (uploadError) throw new Error(uploadError.message);
...
export async function deleteProspectAttachment(id: string) {
  const supabase = createClient();
  const { data: file } = await supabase.from("attachments").select("storage_path").eq("id", id).single();
  if (file) {
    await supabase.storage.from("attachments").remove([file.storage_path]);
