...
// Bulk-apply (or clear) one statut across a set of collaborators x dates in a
// single round trip — used by the planning grid's multi-date range tool.
export async function setPlanningEntriesBulk(employeIds: string[], dates: string[], statut: string | null) {
  ...
  if (!statut) {
    ...
    const rows = employeIds.flatMap((employeId) => dates.map((date) => ({ employe_id: employeId, date, statut, affaire_id: null })));
