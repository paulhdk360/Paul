...
export interface AnniversairesSummary {
  anniversaires: number;
  notified: number;
  error?: string;
}
...
// gets told whose birthday it is. Split out from runRappels so it can be
// triggered on its own, without also touching formations/parc matériel.
export async function runAnniversaires(supabase: SupabaseClient): Promise<AnniversairesSummary> {
  let notified = 0;
  ...
  return { anniversaires: anniversaires.length, notified };
}

// Flags formations/habilitations and parc matériel controls whose deadline
// falls within RAPPEL_WINDOW_DAYS (or is already past), plus runAnniversaires
// above — notifying every admin/direction/administratif_logistique user.
// Shared by the daily cron (/api/cron/rappels, no user session — needs the
// service-role client) and triggerRappelsManually below (already an
...
export async function runRappels(supabase: SupabaseClient): Promise<RappelsSummary> {
  ...
  const anniversairesSummary = await runAnniversaires(supabase);
  notified += anniversairesSummary.notified;

  return {
    formations: formations?.length ?? 0,
    materiels: materiels?.length ?? 0,
    anniversaires: anniversairesSummary.anniversaires,
    notified,
  };
}

const EMPTY_SUMMARY = { formations: 0, materiels: 0, anniversaires: 0, notified: 0 };
const EMPTY_ANNIVERSAIRES = { anniversaires: 0, notified: 0 };
...
// Same idea, but just the birthday check — doesn't touch formations/parc
// matériel deadlines or flip any rappel_envoye flag, so it's safe to fire
// on its own whenever, without side effects on the other reminders.
export async function triggerAnniversairesManually(): Promise<AnniversairesSummary> {
  const { profile } = await requireUser();
  if (profile.role !== "admin") return { ...EMPTY_ANNIVERSAIRES, error: "Réservé aux administrateurs." };
  try {
    return await runAnniversaires(createClient());
  } catch (e) {
