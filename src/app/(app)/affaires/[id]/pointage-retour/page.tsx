...
export default async function PointageRetourPage({ params }: { params: { id: string } }) {
  ...
  const [{ data: affaire }, { data: items }, { data: bls }, { data: outils }, { data: profiles }, { data: commentaires }, { data: purchaseOrders }] =
    await Promise.all([
      ...
      supabase.from("purchase_orders").select("id, numero").eq("affaire_id", params.id),
    ]);
  ...
  return (
    <PointageRetourManager
      ...
      purchaseOrders={(purchaseOrders ?? []) as Pick<PurchaseOrder, "id" | "numero">[]}
    />
  );
}
